    ...
    "/api/auth/register": {
      async POST(req) {
        try {
          ...
          if (!token) {
            return Response.json({ message: "Failed to login after registration" }, { status: 500 });
          }
    ...
    "/api/auth/login": {
      async POST(req) {
        try {
          ...
          return Response.json({ message: error.message || "Login failed" }, { status: 500 });
        }
      }
    },
    ...
    "/api/auth/me": {
      async GET(req) {
        try {
          ...
          return Response.json({ message: error.message || "Authentication failed" }, { status: 500 });
        }
      }
    },
    ...
    "/api/admin/users": {
      async GET(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to get users" }, { status: 500 });
        }
      }
    },
    ...
    "/api/admin/users/:userId/admin-status": {
      async PUT(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to update admin status" }, { status: 500 });
        }
      }
    },
    ...
    "/api/admin/users/:userId/permissions": {
      async PUT(req) {
        try {
          ...
          if (!success) {
            return Response.json({ message: "Failed to update permissions" }, { status: 500 });
          }
          
          return Response.json({ message: "Permissions updated successfully" });
        } catch (error) {
          console.error("Permissions update error:", error);
          return Response.json({ message: error.message || "Failed to update permissions" }, { status: 500 });
        }
      }
    },
    ...
    "/api/admin/default-permissions": {
      async GET(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to get default permissions" }, { status: 500 });
        }
      },
      ...
      async PUT(req) {
        try {
          ...
          if (!success) {
            return Response.json({ message: "Failed to update default permissions" }, { status: 500 });
          }
          
          return Response.json({ message: "Default permissions updated successfully" });
        } catch (error) {
          console.error("Default permissions update error:", error);
          return Response.json({ message: error.message || "Failed to update default permissions" }, { status: 500 });
        }
      }
    },
    // Room routes
    "/api/rooms": {
      async GET(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to get rooms" }, { status: 500 });
        }
      },
      ...
      async POST(req) {
        try {
          ...
          // Check permissions
          if (!user.permissions) {
            return Response.json({ message: "User permissions not found" }, { status: 500 });
          }
          
          const { name, description, isPublic } = await req.json();
          
          if (!name) {
            return Response.json({ message: "Room name is required" }, { status: 400 });
          }
          
          // Check if user can create this type of room
          if (isPublic && !user.permissions.canCreatePublicRoom) {
            return Response.json({ message: "You don't have permission to create public rooms" }, { status: 403 });
          }
          
          if (!isPublic && !user.permissions.canCreatePrivateRoom) {
            return Response.json({ message: "You don't have permission to create private rooms" }, { status: 403 });
          }
          ...
          return Response.json({ message: error.message || "Failed to create room" }, { status: 500 });
        }
      }
    },
    ...
    "/api/rooms/public": {
      async GET(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to get public rooms" }, { status: 500 });
        }
      }
    },
    ...
    "/api/rooms/:roomId": {
      async GET(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to get room details" }, { status: 500 });
        }
      }
    },
    ...
    "/api/rooms/:roomId/join": {
      async POST(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to join room" }, { status: 500 });
        }
      }
    },
    ...
    "/api/rooms/:roomId/messages": {
      async GET(req) {
        try {
          ...
          return Response.json({ message: error.message || "Failed to get room messages" }, { status: 500 });
        }
      },
      ...
      async POST(req) {
        try {
          ...
          if (content && user.permissions && content.length > user.permissions.maxMessageLength) {
    ...
    "/api/dm": {
      async GET(req) {
        try {
          ...
          // Import the function from the API
          const { getDMConversationsForUser } = await import("./lib/api/direct-messages");
          ...
          return Response.json({ message: error.message || "Failed to get DM conversations" }, { status: 500 });
        }
      },
      ...
      async POST(req) {
        try {
          ...
          if (!user.permissions?.canDM) {
            return Response.json({ message: "You don't have permission to send direct messages" }, { status: 403 });
          }
          ...
          // Import the function from the API
          const { getOrCreateDMConversation } = await import("./lib/api/direct-messages");
          
          const conversation = getOrCreateDMConversation(user.id, userId);
          
          if (!conversation) {
            return Response.json({ message: "Failed to create DM conversation" }, { status: 500 });
          }
          
          // Add username for convenience
          const otherUser = db.prepare(`SELECT username FROM users WHERE id = ?`).get(
          ...
          return Response.json({ message: error.message || "Failed to create DM conversation" }, { status: 500 });
        }
      }
    },
    ...
    "/api/dm/:dmId": {
      async GET(req) {
        try {
          ...
          // Import the function from the API
          const { getDMConversation, isUserInDMConversation } = await import("./lib/api/direct-messages");
          
          // Check if user is part of the conversation
          if (!isUserInDMConversation(dmId, user.id)) {
            return Response.json({ message: "You are not part of this conversation" }, { status: 403 });
          }
          ...
          return Response.json({ message: error.message || "Failed to get DM details" }, { status: 500 });
        }
      }
    },
    ...
    "/api/invites/:inviteId": {
      async GET(req) {
        try {
          ...
          if (!invite) {
            return Response.json({ message: "Invite not found or expired" }, { status: 404 });
          }
          
          return Response.json(invite);
        } catch (error) {
          console.error("Invite details error:", error);
          return Response.json({ message: error.message || "Failed to get invite details" }, { status: 500 });
        }
      }
    },
    ...
    "/api/invites/:inviteId/accept": {
      async POST(req) {
        try {
          ...
          if (!result.success) {
            return Response.json({ message: "Failed to use invite" }, { status: 400 });
          }
          
          return Response.json(result);
        } catch (error) {
          console.error("Accept invite error:", error);
          return Response.json({ message: error.message || "Failed to accept invite" }, { status: 500 });
        }
      }
    },
  },
