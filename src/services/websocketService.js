...
async function handleMessage(ws, rawMessage, userId) {
    ...
    switch (event) {
        case 'send_message': {
            const { roomId, encryptedContent, replyToMessageId, mentionedUserIds } = payload;
            if (!roomId || !encryptedContent) {
                return sendToClient(ws, 'error', { message: 'Missing roomId or content for send_message.' });
            }
            ...
            try {
                ...
                if (!isMember) {
                    return sendToClient(ws, 'error', { message: 'You are not a member of this room.' });
                }
        ...
        case 'start_typing': {
            ...
            const { username, display_name } = await require('../models/userModel').findById(userId);
            await broadcastToRoom(roomId, 'user_typing', { roomId, userId, username, displayName: display_name }, userId);
            break;
        }
        ...
        case 'stop_typing': {
            const { roomId } = payload;
            if (!roomId) return;
            await broadcastToRoom(roomId, 'user_stopped_typing', { roomId, userId }, userId);
            break;
        }
        ...
        ws.on('message', async (rawMessage) => {
            ...
            if (!ws.userId) {
                ...
                if (messageData.event === 'auth' && messageData.payload.token) {
                    try {
                        const decoded = jwt.verify(messageData.payload.token, process.env.JWT_SECRET);
                        ws.userId = decoded.userId;

                        // Fetch the rooms this user is part of
                        const userRooms = await Room.getRoomsForUser(decoded.userId);

                        clients.set(ws.userId, { ws, rooms: new Set(userRooms.map(r => r.id)) });
                        clearTimeout(authTimeout);

                        console.log(`✅ WS Client authenticated: User ${ws.userId}`);
                        sendToClient(ws, 'authenticated', { message: 'Authentication successful.' });
                    } catch (error) {
