...
// NOTE: We need a way to get the websocketService to broadcast updates.
// This is a simple service locator pattern for now.
...
exports.addReaction = async (req, res) => {
    ...
    try {
        ...
        await websocketService.broadcastToRoom(messageRoom.room_id, 'reaction_changed', {
            messageId,
...
exports.removeReaction = async (req, res) => {
    ...
    if (!emojiCode) {
     ...
     try {
        const messageRoom = await Message.findRoomForMessage(messageId);
        if (!messageRoom) return res.status(404).json({ message: 'Message not found.' });
