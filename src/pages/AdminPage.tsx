...
export function AdminPage() {
  ...
  const [defaultPermissions, setDefaultPermissions] = useState<DefaultPermissionsType | null>(null);
  ...
  // Set admin status
  const handleSetAdminStatus = async (userId: string, isAdmin: boolean): Promise<boolean> => {
    try {
  ...
  const handleUpdatePermissions = async (userId: string, permissions: any): Promise<boolean> => {
    try {
  ...
  const handleUpdateDefaultPermissions = async (permissions: Partial<DefaultPermissionsType>): Promise<boolean> => {
    try {
      const response = await fetch('/api/admin/default-permissions', {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        // Update local state
        setDefaultPermissions(prev => prev ? { ...prev, ...permissions } : null);
        return true;
      }
      return false;
    } catch (error) {
        ...
        <TabsContent value="permissions">
          {defaultPermissions ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              {isLoadingDefaults ? 'Loading...' : 'Failed to load default permissions'}
            </div>
          )}
        </TabsContent>
      </Tabs>
