...
Session.create = async (userId, token, userAgent, ipAddress) => {
    ...
    const query = `
        ...
        VALUES ($1, $2, $3, $4, NOW());
    `;
    await db.query(query, [userId, tokenHash, userAgent, ipAddress]);
};

Session.findByUserId = async (userId) => {
    const query = 'SELECT id, user_agent, ip_address, last_seen_at, created_at FROM sessions WHERE user_id = $1 ORDER BY last_seen_at DESC';
